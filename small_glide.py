...
I_formula = lambda chord, tau: chord * 0.00025 ** 3 / 12
...
weight = (S + S_h + S_v) * 0.00025 * rho_balsa + H_loc * rho_balsa * 0.0000025 + 0.001
...
opti.subject_to(weight * 9.81 * np.cos(glide_angle * np.pi / 180) < aero["L"])
opti.subject_to(weight * 9.81 * np.sin(glide_angle * np.pi / 180) > aero["D"])
opti.subject_to(aero["CL"] < 1.4)

opti.subject_to(b_total < 0.25)
